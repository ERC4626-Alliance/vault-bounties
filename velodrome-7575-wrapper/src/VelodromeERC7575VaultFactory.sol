...
contract VelodromeERC7575Factory {
    ...
    function createPool(
        ...
        address poolAddress
    ) external returns (address shareToken, address vault0, address vault1) {
        ...
        // Create share token
        shareToken = address(new VelodromeERC7575Share(name, symbol));

        // Create vault for token0
        // TODO: Dynaminally construct vault name and symbol using LibString
        vault0 = address(new VelodromeERC7575Vault(token0,"Vault0","Symbol0", pool, pool.stable(), ERC20((shareToken))));

        // Create vault for token1
        // TODO: Dynaminally construct vault name and symbol using LibString
        vault1 = address(new VelodromeERC7575Vault(token0,"Vault1","Symbol1", pool, pool.stable(), ERC20((shareToken))));
        ...
        emit PoolCreated(shareToken, vault0, vault1, poolAddress);
    }
}